...
public class FavoriteService {
    ...
    public FavoriteService(FavoriteItemRepository favoriteItemRepository, FavoriteCollectionRepository favoriteCollectionRepository,
    ...
    public FavoritesDto favorites(UUID userId, String query, String sort) {
        ...
        List<ItemSummaryDto> items = favoriteItemRepository.findByIdUserId(userId).stream()
                .map(f -> itemService.summary(f.getItem(), userId))
                .filter(item -> q.isBlank() || item.title().toLowerCase().contains(q))
                .sorted(itemComparator(sort))
                .toList();
        List<CollectionSummaryDto> collections = favoriteCollectionRepository.findByIdUserId(userId).stream()
                .map(f -> collectionService.summary(f.getCollection()))
                .filter(c -> q.isBlank() || c.title().toLowerCase().contains(q))
                .sorted(collectionComparator(sort))
                .toList();
    ...
    private Comparator<ItemSummaryDto> itemComparator(String sort) {
        return switch (sort == null ? "new" : sort) {
            ...
            default -> Comparator.comparing(ItemSummaryDto::createdAt, Comparator.nullsLast(Comparator.naturalOrder())).reversed();
        };
    }
    ...
    private Comparator<CollectionSummaryDto> collectionComparator(String sort) {
        return switch (sort == null ? "new" : sort) {
            ...
            default -> Comparator.comparing(CollectionSummaryDto::createdAt, Comparator.nullsLast(Comparator.naturalOrder())).reversed();
        };
    }
}
