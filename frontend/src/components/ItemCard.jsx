...
function ItemCard({ item, compact = false, readonly = false, onRemoveFromCollection }) {
  ...
  return (
    <article className={`item-card ${compact ? 'item-card--compact' : ''}`}>
      <Link to={`/items/${item.id}`} className="item-card__visual-link" aria-label={`Открыть предмет ${item.title}`}>
        ...
        {canEdit && (
          <div className="item-card__actions">
            ...
            {onRemoveFromCollection && (
              <Button variant="ghost" onClick={() => onRemoveFromCollection(item.id)}>Убрать из коллекции</Button>
            )}
