    ...
    try {
      ...
      if (imageFile) {
        try {
          ...
          coverImageUrl = imagePreview || coverImageUrl
        }
      }
      await dispatch(addCollection({ ...form, coverImageUrl })).unwrap()
    } catch (requestError) {
  ...
  return (
    <form className="form-grid" onSubmit={submitForm}>
      ...
      <div className="form-grid__wide upload-preview">
        ...
        >
          {imagePreview ? <img src={imagePreview} alt="Предпросмотр коллекции" /> : <span>Фото не выбрано</span>}
        </div>
      ...
      <div className="form-grid__actions">
        <Button variant="primary" type="submit" disabled={isSubmitting}>{isSubmitting ? 'Сохранение...' : 'Сохранить коллекцию'}</Button>
      </div>
    </form>
  )
}
